...
describe('Nested Loop Cleanup', () => {
    ...
    test('should destroy inner loop elements when outer loop updates', async () => {
        ...
        expect(root.componentInstance.children[0].name).toBe('inner-1')
        expect(root.componentInstance.children[1].name).toBe('inner-2')
        ...
        await vi.waitFor(() => {
            ...
            // If bug exists, old children might remain
            expect(root.componentInstance.children.length).toBe(1)
            expect(root.componentInstance.children[0].name).toBe('inner-3')
        })
    })
})
